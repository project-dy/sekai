...
class Util {
	...
	async downloadAllAudio(chartFile, folderPath, until = 100) {
		const chart = JSON.parse(fs.readFileSync(chartFile)).data.slice(0, until);
		const audioList = [];
	...
	async downloadAudio(rank, name, artist, audioPath) {
